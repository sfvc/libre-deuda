...
function SearchInfractor ({ resetFiltro, onSelectPersona }) {
  ...
  useEffect(() => {
    ...
    const valor = cuit || patente
    if (valor && valor.length > 2) {
      setSearchInput(valor)
      setHasSelectedPersona(false)
      setShow(true)

      const valorCorto = valor.slice(0, -1)
      setSearch(valorCorto)
  ...
  useEffect(() => {
    ...
    const cuit = params.get('cuit')
    const patente = params.get('patente')
    const valorInicial = cuit || patente

    if (valorInicial && personas.length === 1 && !hasSelectedPersona && !isFetching) {
      const personaEncontrada = personas[0]
      selectPersona(personaEncontrada)
    }
  }, [personas, hasSelectedPersona, isFetching])
